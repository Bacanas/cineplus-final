    ...
    routes.get('/busca/:pesquisa', async (req,res)=>{
        ...
        //console.log(`SELECT Filmes.id_filme, Filmes.capa, Filmes.titulo, Filmes.ano, Diretores.nome AS nome_diretor, Generos.nome AS nome_genero FROM Filmes JOIN Diretores ON Filmes.id_diretor = Diretores.id_diretor JOIN Generos ON Filmes.id_genero = Generos.id_genero WHERE Filmes.titulo LIKE '` + busca + `%'`)
  
        try{
            const {pesquisa }  = req.params
            const { recordset }  = await pool.query("SELECT Filmes.id_filme, Filmes.capa, Filmes.titulo, Filmes.ano, Diretores.nome AS nome_diretor, Generos.nome AS nome_genero FROM Filmes JOIN Diretores ON Filmes.id_diretor = Diretores.id_diretor JOIN Generos ON Filmes.id_genero = Generos.id_genero WHERE Filmes.titulo LIKE '" + pesquisa + "%'")
